...
export default class SprintResolver {
    ...
    getAllTicketsBySprint(@Arg("sprint") sprint:Sprint){
    ...
    getAllCurrentSprints(){
        const sprintRepository:Repository<Sprint> = getRepository(Sprint);
        const sprints = sprintRepository.createQueryBuilder("sprint")
        .where("sprint.start_date IS NOT NULL" && "sprint.end_date IS NOT NULL" && "sprint.start_date<Date.now()" &&
        "sprint.end_date>Date.now()").getMany();
        return sprints;
    }
    ...
    getAllPastSprints(){
        const sprintRepository:Repository<Sprint> = getRepository(Sprint);
        const sprints = sprintRepository.createQueryBuilder("sprint")
        .where("sprint.end_date IS NOT NULL" && "sprint.end_date<Date.now()").getMany();
        return sprints;
    }
    ...
    getAllFutureSprints(){
        const sprintRepository:Repository<Sprint> = getRepository(Sprint);
        const sprints = sprintRepository.createQueryBuilder("sprint")
        .where("sprint.start_date IS NOT NULL" && "sprint.start_date>Date.now()").getMany();
        return sprints;
    }
    ...
    createSprint(@Arg("start_date") start_date:Date, @Arg("end_date") end_date:Date, @Arg("tickets") tickets:Ticket[]){
        const sprintRepository:Repository<Sprint> = getRepository(Sprint);
    ...
    @Mutation(returns => Sprint)
    async updateSprint(@Arg("id") id:number, @Arg("start_date") start_date:Date, @Arg("end_date") end_date:Date,
